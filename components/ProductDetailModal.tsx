...
const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ 
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    if (isOwner) {
      onShowToast?.("Sellers cannot review their own products.", "error");
      return;
    }
    ...
    if (result) {
      ...
      setReviewComment('');
      onShowToast?.("Review posted successfully!", "success");
    }
      ...
      <div className="bg-slate-900 w-full max-w-4xl h-full sm:h-[92vh] sm:rounded-[40px] shadow-2xl border border-white/5 overflow-y-auto overflow-x-hidden custom-scrollbar scroll-smooth flex flex-col relative" dir="ltr">
        ...
        <div className="relative z-10 bg-slate-900 p-6 sm:p-10 space-y-10 text-left">
          <div className="flex flex-col gap-6">
            ...
            {/* NEW: Reviews Summary Box directly under seller info */}
            <div className="bg-indigo-500/5 rounded-[35px] border border-indigo-500/10 p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl relative overflow-hidden group">
               <div className="absolute top-0 right-0 p-10 opacity-[0.03] pointer-events-none group-hover:scale-110 transition-transform duration-700">
                  <svg className="w-24 h-24 text-indigo-500 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
               </div>
               <div className="flex items-center gap-5">
                  <div className="w-14 h-14 bg-slate-950 rounded-2xl flex items-center justify-center border border-white/5 shadow-inner">
                     <span className="text-xl font-black text-amber-400">{reviews.length > 0 ? (reviews.reduce((acc, curr) => acc + curr.rating, 0) / reviews.length).toFixed(1) : '0.0'}</span>
                  </div>
                  <div>
                    <h4 className="text-[11px] font-black text-white uppercase tracking-widest mb-1">Product Reputation</h4>
                    <div className="flex items-center gap-2">
                       {renderStarRating(reviews.length > 0 ? (reviews.reduce((acc, curr) => acc + curr.rating, 0) / reviews.length) : 0, "w-3.5 h-3.5")}
                       <span className="text-[9px] text-slate-500 font-black uppercase">({reviews.length} Feedbacks)</span>
                    </div>
                  </div>
               </div>
               {!isOwner && currentUserEmail && (
                 <button 
                   ...
                   className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white text-[9px] font-black rounded-xl border border-white/10 shadow-lg transition-all active:scale-95 uppercase tracking-widest"
                 >
                   Write a Review
                 </button>
               )}
               {isOwner && (
                 <div className="px-5 py-2.5 bg-slate-800/40 text-slate-500 text-[8px] font-black rounded-lg border border-white/5 uppercase tracking-widest">
                   Viewing as Owner
                 </div>
               )}
            </div>
